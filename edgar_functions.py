...
def get_quarterly_facts_for_ticker(ticker: str) -> pd.DataFrame:
    ...
    quarterly_pivot = quarterly_facts.pivot_table(
...
def get_annual_facts_for_ticker(ticker: str) -> pd.DataFrame:
    ...
    annual_pivot = annual_facts.pivot_table(
        index="filed", columns="account_key", values="val"
    ).sort_index(ascending=False)
...
def get_statement_soup(ticker, accession_number, statement_name):
    ...
    headers = {"User-Agent": "russ@sunriseanalysis.com"}
    base_link = f"https://www.sec.gov/Archives/edgar/data/{cik}/{accession_number}"
    statement_file_names_dict = get_statement_file_names_in_filing_summary(
        ...
        for row in table.select("tr"):
            ...
            for i, cell in enumerate(row.select("td.text, td.nump, td.num")):
                ...
                if "nump" in cell.get("class"):
                    values[i] = (
                        value / unit_multiplier if unit_multiplier != 1 else value
                    )
                else:
                    values[i] = (
                        -value / unit_multiplier if unit_multiplier != 1 else -value
...
def form_statement_for_all_available_quarters(ticker, statement_name):
    ...
    for accession_number in accession_number_series:
        df = process_one_statement(ticker, accession_number, statement_name)
        all_available_quarters = pd.concat(
            [all_available_quarters, df], axis=0, join="outer"
        )
        print(
            f"{statement_name} for accession number: {accession_number} has been processed."
        )
        ...
        for gaap_name in gaap_names:
            if gaap_name in statement.columns:
                if new_statement[common_name].isnull().all():
                    ...
                    new_statement[common_name].fillna(
                        statement[gaap_name], inplace=True
                    )

    return new_statement


def create_scorecard_from_simple_statements(
    ...
    scorecard["Marketable Securities"] = balance_sheet["Marketable Securities"]
    ...
    scorecard["Long Term Marketable Securities"] = balance_sheet[
    ...
    scorecard["Commercial Paper"] = balance_sheet["Commercial Paper"]
    scorecard["Long Term Debt Current"] = balance_sheet["Long Term Debt Current"]
    ...
    scorecard["Long Term Debt"] = balance_sheet["Long Term Debt"]
    scorecard["Noncurrent Liabilities"] = balance_sheet["Noncurrent Liabilities"]
...
def retrieve_balance_income_cf_and_store_as_CSV(ticker, blank: bool = True):
    ...
    cash_flow_sheet = form_statement_for_all_available_years(
    ...
    save_dataframe_to_csv(
        cash_flow_sheet, "results", ticker, "cash_flow_statement", "annual"
    )
    save_dataframe_to_csv(balance_sheet, "results", ticker, "balance_sheet", "annual")
    save_dataframe_to_csv(
        earnings_sheet, "results", ticker, "income_statement", "annual"
    )
    save_dataframe_to_csv(
        all_three_statements, "results", ticker, "all_three_main_statements", "annual"
    )
...
def store_balance_income_cf_for_all_quarters_and_years(ticker):
    ...
    save_dataframe_to_csv(
        cash_flow_sheet,
        "quarterly&annual_results",
        ticker,
        "cash_flow_statement",
        "annual",
    )
    save_dataframe_to_csv(
        balance_sheet, "quarterly&annual_results", ticker, "balance_sheet", "annual"
    )
    save_dataframe_to_csv(
        earnings_sheet, "quarterly&annual_results", ticker, "income_statement", "annual"
    )
    earnings_sheet = form_statement_for_all_available_quarters(
        ticker, "income_statement"
    )
    balance_sheet = form_statement_for_all_available_quarters(ticker, "balance_sheet")
    cash_flow_sheet = form_statement_for_all_available_quarters(
        ticker, "cash_flow_statement"
    )
    save_dataframe_to_csv(
        cash_flow_sheet,
    ...
    save_dataframe_to_csv(
        balance_sheet, "quarterly&annual_results", ticker, "balance_sheet", "quarterly"
    )
    save_dataframe_to_csv(
        earnings_sheet,
        "quarterly&annual_results",
...
def print_links_to_desired_statment(ticker, statement_name):
    ...
    for accession_number in acc_series:
        ...
        try:
            ...
            statement_link = None
            for possible_key in statement_keys_map.get(
                statement_name, [statement_name]
            ):
                try:
                    statement_link = (
                        f"{base_link}/{statement_file_names_dict[possible_key]}"
                    )
                    break
                except KeyError:
                    continue
